...
class RobotSiguelineasBusqueda(Node):
    def __init__(self):
        ...
        try:
            ...
            time.sleep(2)
            # Limpiamos buffers para evitar que el primer comando llegue roto
        ...
        # NUEVA POSICIÓN DE BÚSQUEDA Y POSICIÓN DE AGARRE (Sin intermedias)
        self.POS_BUSCADOR = [0.13345632854584366, 1.541650691824862, -1.7088545977048113]
        self.POS_AGARRE = [1.535515, 0.483204, -1.98190]

        # --- 3. PARAMETROS ARUCO (CALIBRADOS) ---
        self.TARGET_ID = 36
        self.TARGET_AREA_MIN = 21000.0  
        self.TARGET_ERR_X_MIN = 50      
        self.TARGET_ERR_X_MAX = 85      
...
def main():
    ...
    # --- PURGA DE LA CÁMARA ---
    print("[INFO] Purgando sensor de la camara para ajuste de luz...")
    for _ in range(40):  
        ret, _ = cap.read()
        time.sleep(0.01) 
    ...
    try:
        while rclpy.ok():
            ...
            if contornos_validos:
                # SUPERVIVENCIA DEL MÁS CENTRADO
                ...
                # --- EMBRAGUE: Evitar tirones en el arranque ---
                if frames_procesados < 10:
                    robot.parar_motores()
                    print(f"Estabilizando vision... ({frames_procesados}/10)             ", end='\r')
                else:
            ...
            # --- DETECCIÓN ALMACÉN VERDE (SOLO SI LLEVA PIEZA) ---
            if carrying:
                ...
                if detected_green and not green_current and now >= green_lock_until:
                    ...
                    if green_seen_count == 1:
                        print("\nAlmacen detectado: 1/2 -> siguiendo")
                    else:
                        print(f"Almacen detectado: {green_seen_count}/2")
                
                green_current = detected_green
                
                if green_seen_count >= 2:
                    print("\n[INFO] Almacen 2/2 detectado -> Dejando pieza")
                    robot.parar_motores()
                    time.sleep(0.5)
                    # Avance recto para meter el morro en el almacén
                    robot.avanzar_ms(2000, velocidad=25)
                    robot.soltar_pieza()
                    robot.parar_motores()
                    break

            # --- DETECCION ARUCO (SOLO SI NO LLEVA PIEZA) ---
            if not carrying and frames_procesados >= 10:
                ...
                if ids is not None and robot.TARGET_ID in ids:
                    ...
                    centro_full = frame_full.shape[1] // 2
                    error_x_aruco = cx_aruco - centro_full
                    
                    # LA CONDICIÓN DE ORO: Tamaño y Alineación perfectas
                    if area >= robot.TARGET_AREA_MIN and (robot.TARGET_ERR_X_MIN <= error_x_aruco <= robot.TARGET_ERR_X_MAX):
                        print(f"\n¡BLANCO FIJADO! Area: {area:.0f} | ErrX: {error_x_aruco}")
                        robot.parar_motores()
                        time.sleep(1.0) 
